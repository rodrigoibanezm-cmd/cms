...
function classifier(value, family) {
  ...
  source(value.source_field, `${family}.classifier.source_field`)
  return value
}
function template(value, family) {
  ...
  text(value.filename, `${family}.template_reference.filename`)
  text(value.drive_file_id, `${family}.template_reference.drive_file_id`)
...
function mapping(value, family) {
  ...
  value.operations.forEach((operation, index) => {
    ...
    source(operation.source, `${field}.source`)
    if (!CELL.test(text(operation.target, `${field}.target`))) throw new Error(`${field}.target is invalid.`)
  })
  return value
}
function metadata(value, family) {
  ...
  if (value.mime_type !== XLSX_MIME) throw new Error(`${family}.render_metadata.mime_type is unsupported.`)
  return value
}

function certifyFamily(family, aliases, sourceValue) {
  const value = object(sourceValue, `Family ${family}`)
  exactKeys(value, ['source_ref', 'classifier', 'template_reference', 'render_mapping', 'render_metadata'], `Family ${family}`)
  if (!Array.isArray(aliases) || !aliases.length) throw new Error(`Family ${family} is not executable. Missing aliases.`)
  return {
    source_ref: text(value.source_ref, `Family ${family}.source_ref`), aliases: [...aliases],
    classifier: classifier(value.classifier, family), template_reference: template(value.template_reference, family),
    render_mapping: mapping(value.render_mapping, family), render_metadata: metadata(value.render_metadata, family),
  }
}

module.exports = { REPORT_SOURCES, certifyFamily }
