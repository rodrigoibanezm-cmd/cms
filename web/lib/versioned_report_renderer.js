...
async function persistResult({ report, catalogVersion, familyKey, contract, xls }) {
  await query(`UPDATE reports SET template_key=$2, template_filename=$3,
    excel_url=$4, drive_file_id=$5, status='processed', updated_at=now()
    WHERE id=$1`, [report.id, familyKey, contract.template_reference.filename,
    xls.excel_url, xls.drive_file_id]);
  ...
  await addReportEvent(report.id, 'versioned_xls_rendered', {
    catalog_version_id: catalogVersion.id,
    catalog_version: catalogVersion.version,
    catalog_hash: catalogVersion.catalog_hash,
    family: familyKey,
...
export async function renderReport({ reportId, catalogVersionId }) {
  ...
  const context = { report, extraction: report.extraction_json };
  const family = selectCertifiedFamily(catalogVersion.compiled_catalog, context);
  const extraction = {
