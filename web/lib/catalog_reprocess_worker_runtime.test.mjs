...
function poolWithClaim(request, rowCounts = [], queryError = null) {
  const calls = [];
  const client = {
    query: async (sql) => {
      calls.push(sql);
...
test('error SQL al persistir failed conserva el error de render', async () => {
  const persistenceError = new Error('database unavailable');
  const pool = poolWithClaim(request('request-5'), [], persistenceError);
  const renderError = new Error('render failed');
  const processNext = createCatalogReprocessWorker({
    pool,
    renderReport: async () => { throw renderError; },
  });
  await assert.rejects(() => processNext(), (error) => {
    assert.match(error.message, /could not be persisted: database unavailable/);
