...
async function fail(pool, request, renderError) {
  const payload = {
  ...
  let update;
  try {
    update = await pool.query(`UPDATE catalog_reprocess_requests
      SET status='failed', failed_at=now(), error_json=$2
      WHERE id=$1 AND status='processing'`, [request.id, JSON.stringify(payload)]);
  } catch (persistenceError) {
    const error = new Error(
      `Catalog reprocess request transition to failed could not be persisted: ${persistenceError.message}`,
      { cause: renderError },
    );
    error.persistenceError = persistenceError;
    throw error;
  }
  assertTransition(update, 'failed', renderError);
}
